...
class BookingProvider extends ChangeNotifier {
  ...
  Future<void> fetchBookings() async {
    ...
    try {
      ...
      // FIX: Fetch bookings, including pet details (name, type, BREED) and procedures
      final List<Map<String, dynamic>> data = await _client
          .from('bookings')
          .select('*, pets(name, type, breed), procedures') // ADDED 'breed' here
          .eq('owner_id', userId)
          .order('start_date', ascending: true);
      ...
      _errorMessage = 'Failed to load bookings: $e';
      _allBookings = []; // Clear bookings on error
      debugPrint('Error fetching bookings in BookingProvider: $e'); // Use debugPrint
    } finally {
      _isLoading = false;
      notifyListeners(); // Notify listeners that loading has finished
    }
  ...
  // You can add methods here for adding, updating, deleting bookings
  // Example:
  // Future<void> addBooking(Map<String, dynamic> newBookingData) async {
  //   try {
  //     // ... Supabase insert logic ...
  //     await fetchBookings(); // Refresh the list after adding
  //   } catch (e) {
  //     // Handle error
  //   }
  // }
