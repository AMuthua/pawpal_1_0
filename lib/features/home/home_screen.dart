...
import 'package:pawpal/theme/theme_service.dart'; // <--- NEW: Import ThemeService
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    final userEmail = Supabase.instance.client.auth.currentUser?.email ?? 'Guest';
    final themeService = Provider.of<ThemeService>(context, listen: false); // <--- NEW: Access ThemeService

    return Scaffold(
      appBar: AppBar(
        title: const Text('Your Pet Dashboard'),
        // <--- NEW: Add dark mode toggle to AppBar actions
        actions: [
          IconButton(
            icon: Icon(
            ...
            onPressed: () {
              ...
              } else { // If system theme, toggle to light/dark based on current brightness
          ...
          // <--- NEW: Option to set to System default (optional)
          TextButton(
            onPressed: () {
              themeService.setThemeMode(ThemeMode.system);
            },
            child: const Text('System', style: TextStyle(color: Colors.white)), // Ensure text color contrasts with AppBar
          )
        ],
        // <--- END NEW AppBar actions
      ),
      drawer: Drawer(
        child: ListView(
          padding: EdgeInsets.zero,
          children: [
            ...
            _buildDrawerItem(Icons.support_agent, 'Customer Support', () => context.push('/support')),
            const Divider(),
            // <--- NEW: Add a Settings drawer item
            _buildDrawerItem(Icons.settings, 'Settings', () {
              // You can navigate to a dedicated settings screen here
              // For now, let's just show a snackbar or go to a placeholder route
              _showSnackBar('Settings page coming soon!');
              // context.push('/settings'); // Uncomment this if you create a /settings route
            }),
            _buildDrawerItem(Icons.logout, 'Logout', () => _logout(context)),
          ],
        ),
  ...
  // <--- NEW: Add _showSnackBar method here for consistency
